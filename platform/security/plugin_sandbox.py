...
class PluginSandboxBroker:
 def __init__(self,policy:PluginSandboxPolicy):self.policy=policy
 ...
 def run_process(self,command:Sequence[str],*,timeout_seconds:int=30)->subprocess.CompletedProcess[bytes]:
  if not command or command[0] not in self.policy.allowed_executables:raise PluginSandboxError("plugin executable is not authorized")
  if timeout_seconds<1 or timeout_seconds>300:raise PluginSandboxError("plugin process timeout is invalid")
