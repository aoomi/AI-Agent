...
class AgentMapper:
    ...
    def bind(self, skill: SkillDefinition, agent: AgentInstance) -> None:
        ...
        current_agent = self._skill_to_agent.get(skill.skill_id)
        current_skill = self._agent_to_skill.get(agent.agent_id)
        if current_agent not in (None, agent) or current_skill not in (None, skill):
            raise AgentMappingError("Skill and agent mapping must remain one-to-one")
        self._skill_to_agent[skill.skill_id] = agent
        self._agent_to_skill[agent.agent_id] = skill

    def agent_for(self, skill_id: str) -> AgentInstance:
        try:
            return self._skill_to_agent[skill_id]
        except KeyError as error:
            raise AgentMappingError(f"unmapped skill_id: {skill_id}") from error

    def skill_for(self, agent_id: str) -> SkillDefinition:
        try:
            return self._agent_to_skill[agent_id]
        except KeyError as error:
            raise AgentMappingError(f"unmapped agent_id: {agent_id}") from error
