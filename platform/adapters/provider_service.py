...
class ProviderService:
    def __init__(self,checker:ProviderHealthChecker|None=None):
        ...
        self.checker=checker;self.configurations={};self.health={};self._lock=RLock();self._inflight:set[str]=set()
    def register(self,*,provider_id:str,display_name:str,kind:str,endpoint:str,secret_reference:str,capabilities:tuple[str,...],enabled:bool=True,timeout_seconds:int=60,settings:Mapping[str,Any]|None=None,replace:bool=False)->ProviderConfiguration:
        ...
        with self._lock:
            ...
            if existing and provider_id in self._inflight:raise ProviderServiceError("provider has in-flight health check")
            now=self._now();item=ProviderConfiguration(provider_id,display_name,kind,endpoint,secret_reference,capabilities,enabled,timeout_seconds,MappingProxyType(json.loads(canonical_settings)),existing.created_at if existing else now,now);self.configurations[provider_id]=item;self.health[provider_id]=ProviderHealth(provider_id,"unknown",now,None,None,0);return item
    def unregister(self,provider_id:str)->bool:
        ...
        with self._lock:
            if provider_id in self._inflight:raise ProviderServiceError("provider has in-flight health check")
            removed=self.configurations.pop(provider_id,None);self.health.pop(provider_id,None);return removed is not None
    ...
    def test_connection(self,provider_id:str)->ProviderHealth:
        ...
        with self._lock:
            ...
            if provider_id in self._inflight:raise ProviderServiceError("provider health check is already active")
            self._inflight.add(provider_id)
        try:
            ...
            with self._lock:self._inflight.discard(provider_id)
    @staticmethod
