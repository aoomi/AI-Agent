...
class IndustryWorkflow:
 workflow_id:str;industry_id:str;robot_ids:tuple[str,...];edges:tuple[tuple[str,str],...];mode:str;version:int=1
class IndustryWorkflowService:
 ...
 def execute(self,configuration,changes:Mapping[str,Any],confirmed_by_identity_id:str)->Mapping[str,Any]:
  ...
  if operation=="create":
   ...
   item=IndustryWorkflow(workflow_id,industry,robots,(),mode);self.workflows[workflow_id]=item
  else:
