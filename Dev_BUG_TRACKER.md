...
- 稽查整改独立软件复测：通过。冻结提交`7ccc1b7`关联`114 passed`无失败无跳过；Python编译、Vue类型检查和文档状态门禁通过。
- 最终只读复稽查：通过。列表、创建、专用媒体和删除均在访问数据前验证完整所有者作用域；project/project_episode强制项目归属，旧资源URL安全投影且通用媒体入口无法读取resources目录。空身份、跨tenant/user/project及受控根越界全部失败关闭。
- 关闭时间：2026-08-12（Asia/Shanghai）。下一状态：已关闭；继续插件/provider切换保护演练。
...
- 稽查整改独立软件复测：通过。冻结提交`749875b`关联可观测、生产控制面和文档状态`114 passed`，无失败无跳过；Python编译通过。动态新增EOF释放验证，不增加请求计数且活动数归零。
- 最终只读复稽查：通过。正式服务仅在当前请求解析后接受有界关联头，每请求代际隔离；JSONL包含关联事件但不接收秘密字段，Prometheus只含有界方法/状态类标签并原子替换。正常、异常状态与keep-alive EOF均保持活动计数守恒，启停生命周期接线完整。
- 关闭时间：2026-08-12（Asia/Shanghai）。下一状态：已关闭；继续现有身份隔离纵深验收。

### BUG-20260812-066：单节点混合负载缺少可重复容量基准

- 状态：已关闭（最终只读复稽查通过）
- 关联任务：M10.7 / V1单节点控制面、背压和资源释放验收，不启动H3或其他重模型。
- 正式复现：资源池、队列上限和四层背压已有单元测试，但仓库没有可重复混合负载基准脚本或报告；NFR中的API延迟、队列年龄、取消时延、吞吐和资源释放阈值仍全部“待真实基准写回”。
- 首个事实：现有测试能证明单个调度性质，不能同时测量accelerator长任务占用时control/cpu-media是否被饿死、同/跨租户背压、取消唤醒时延和测试结束后的线程/票据清零。
- 风险：单机V1无法给出容量配置证据，回归可能引入控制请求饥饿、无界排队或取消延迟而CI仍通过。
- 整改标准：实现无重模型的可重复单节点混合负载基准，输出吞吐、P50/P95/P99、队列年龄、取消时延、背压计数、峰值活动/排队和最终资源快照；门禁只采用本机实测且留足稳定裕量，硬件/参数写回NFR，不伪装为生产SLA。
- 主线实现：新增可重复benchmark并使用正式`ResourceScheduler`同时运行80 control、24 cpu-media和8 accelerator模拟任务，容量固定4/2/1；单独动态覆盖pool、tenant、project三层排队拒绝和排队任务取消。报告包含环境/参数、吞吐、各池P50/P95/P99/max等待、队列年龄、取消时延、峰值活动/排队、线程及所有最终快照；门禁要求control P99<500ms、取消<500ms、三层背压各一次且票据/线程归零。
- 开发验证：Darwin arm64 / Python 3.11.15正式参数实测吞吐939.06 req/s；control P50/P95/P99=0.645/1.508/1.582ms，cpu-media P99=41.466ms，accelerator排队最大102.853ms，取消0.050ms；峰值活动7、排队29，三层背压均拒绝1次，所有池和benchmark线程归零。基准/文档`21 passed`，无失败无跳过；未启动模型或正式任务。
- 首轮独立软件测试：冻结提交`e8f8f0c`基准/文档`21 passed`无失败无跳过；正式参数复跑963.54 req/s、control P99 1.482ms、取消0.033ms，三层背压及资源/线程归零一致。
- 首轮只读稽查：不通过（P1）。报告虽记录峰值活动数，却未把“各池总活动不得超过容量总和”纳入validate门禁；未来调度器超卖时可能仍以低延迟通过。
- 稽查整改：validate新增峰值活动≤配置容量总和，动态测试同步断言；当前峰值7等于4+2+1合法上限，待独立复测。
- 稽查整改独立软件复测：通过。冻结提交`d674435`复跑基准、文档契约和状态源`22 passed`，无失败无跳过；正式参数957.93 req/s、control P99 1.607ms、取消0.040ms，峰值活动7等于容量总和，三层背压及全部票据/线程归零。
- 最终只读复稽查：通过。基准直接调用正式`ResourceScheduler`，三类资源池隔离、pool/tenant/project背压、排队取消唤醒、峰值不超卖及终态资源归零均进入失败关闭门禁；硬件和参数完整记录，当前数值明确仅为本机小样本回归基线，不冒充生产SLA。
- 关闭时间：2026-08-12（Asia/Shanghai）。下一状态：已关闭；继续处理不依赖CUDA的框架生产化缺口。

### BUG-20260812-065：备份恢复缺少一致性快照与逐文件完整性门禁

- 状态：已关闭（最终只读复稽查通过）
- 关联任务：M10.5 / 单节点V1灾难恢复，不依赖BUG057 CUDA提供方，不操作正式数据。
- 正式复现：`backup_restore.py`直接把活动目录加入tar，仅对整个压缩包做一次`read_bytes()`哈希；没有SQLite在线一致性快照、逐文件清单、符号链接拒绝、恢复后数量/哈希复核或原子发布。现有集成测试只覆盖两个静态文件和尾部篡改。
- 首个事实：活动SQLite在tar逐文件读取期间可跨事务页变化，生成内部不一致备份；合法哈希的归档仍可包含越界语义的符号链接、缺文件或被替换内容，恢复后没有证据发现。
- 风险：单机V1故障时可能得到“归档校验通过但数据库不可用/媒体缺失”的假备份，且非原子归档或manifest可被并发恢复读取。
- 整改标准：只处理五类允许根；SQLite使用在线backup API取得一致性副本；拒绝源/归档符号链接和特殊文件；流式生成归档及逐文件size/SHA-256清单并原子发布，恢复后精确复核集合、大小和哈希。使用临时隔离目录完成真实备份→恢复→SQLite查询与媒体哈希演练。
- 主线实现：备份先在输出同盘临时目录构建五类白名单快照，SQLite使用只读在线backup API且排除WAL/SHM/journal边车；源和归档的软/硬链接、特殊文件、重复/越界成员及非白名单根全部拒绝。manifest v2记录流式归档哈希及逐文件路径/大小/哈希，归档与manifest经`fsync + os.replace`发布；恢复在同盘临时目录逐成员安全提取并精确复核inventory后才切换空目标。
- 开发验证：备份/恢复、篡改拒绝、SQLite可查询、运行边车排除、源链接拒绝、归档链接拒绝及manifest内容篡改`26 passed`，无失败无跳过。隔离演练在WAL持续写入时备份200条初始记录与1MiB媒体，0.0076秒完成；恢复SQLite `integrity_check=ok`且可查询471条，媒体SHA-256一致，manifest精确2文件。该小样本不是生产RPO/RTO承诺。
- 首轮只读稽查：不通过（P1）。内部成员链接已拒绝，但backup/restore在检查前先对根路径`resolve()`；若备份源、归档、manifest、输出或恢复目标本身是符号链接，会丢失链接身份并跟随到外部真实目录，绕过“源和恢复路径不允许链接”的边界。
- 稽查整改：所有根路径在`resolve()`前检查symlink；备份源/输出链接拒绝为`INVALID_BACKUP_SOURCE`，归档/manifest/恢复目标链接拒绝为`UNSAFE_RESTORE_PATH`。动态证明链接恢复目标外部目录保持空；关联更新为`27 passed`，待独立复测。
- 首轮整改独立复测：通过。冻结提交`d0fde8a`关联`27 passed`无失败无跳过，Python编译通过。
- 二轮只读稽查：不通过（P1）。手工安全提取没有恢复文件mode，原`0600`配置/秘密引用会按进程umask变成更宽权限；manifest也未登记mode，恢复后无法发现权限漂移。
- 二轮稽查整改：隔离快照保留SQLite文件、普通文件和目录元数据；manifest v2逐文件增加权限mode，安全提取后显式应用tar内mode并由inventory复核。动态证明`0600`秘密引用恢复后仍为`0600`；关联`28 passed`，待独立复测。
- 二轮整改独立复测：通过。冻结提交`cb79cb5`关联`28 passed`无失败无跳过，Python编译通过。
- 三轮只读稽查：不通过（P1）。文件mode已恢复，但目录成员只`mkdir`未应用归档mode；原`0700 config`会恢复为umask默认目录，仍扩大秘密引用可见范围。
- 三轮稽查整改：安全提取记录目录mode，全部文件写完后按逆层级应用，既不让只读目录阻断恢复写入，又保持最终权限。动态同时断言config目录`0700`和秘密引用`0600`；关联仍为`28 passed`，待独立复测。
- 三轮整改独立复测：通过。冻结提交`090a7f0`复跑SQLite一致性、完整性、安全路径、文件/目录权限及文档状态`28 passed`，无失败无跳过；Python编译通过。
- 最终只读复稽查：通过。白名单快照不跟随任何层级链接，SQLite在线副本不混入运行边车；归档与manifest流式哈希、fsync和原子发布，恢复只向同盘临时目录写入。成员类型/路径/重复项和逐文件集合、大小、mode、哈希均失败关闭，文件与目录权限恢复后可验证，异常不发布部分目标。隔离WAL演练与测试证据一致且未宣称生产RPO/RTO。
- 关闭时间：2026-08-12（Asia/Shanghai）。下一状态：已关闭；进入M10.7单节点混合负载容量基准。
...
- 当前阻塞更新（2026-08-12）：image前序、Context IR、嵌套3D媒体解析及设备门禁均已闭环；本机Comfy仅MPS，而固定H3 INT8 ConvRot制品只支持CUDA。正式任务已证明MPS会落入单核CPU且70分钟0/20，现按BUG062在提交任何prompt前持久`model_blocked`。缺少CUDA兼容H3执行提供方，无法产生真实静音H3媒体及ffprobe证据；不得改用Wan或模拟产物冒充。
- 用户验收口径变更（2026-08-12）：明确授权改用本机现有视频模型，不再要求H3；只验证2—3镜头、总时长≤15秒的视频，不验证人声与BGM。Wan证据必须按自身provider登记，不冒充H3结果。
- 本地Wan首轮正式复现：隔离项目通过正式`/api/videos/generate`串行生成shot 1/2，各3.06秒、49帧、576×1024、16fps、H.264且无音轨，job `9b87294c-2591-4b4c-b049-0758ddbfe167`与`a90c9971-6e36-4bbe-8ba2-39a3ed4f722a`均completed，资源和Comfy队列归零；但逐帧检查发现约1.5秒后高光溢出、结构拉丝和主体消失，结论不通过。
...
- 正式根因：`/api/assistant/history`按tenant/user/current_project/session精确键查询失败时，只要整个存储恰有一份`display_history`，就无条件返回那份其他项目历史。新项目或新会话因此会恢复旧项目消息及其图片URL；前端项目缓存即使正确隔离，也会被服务端错误响应重新污染。
- 框架整改：删除单条历史兼容回退，新增纯函数`_conversation_messages`只接受精确上下文键且值必须为消息数组；缺键、其他项目唯一历史、畸形history容器或畸形投影均返回空数组。保存边界与现有项目缓存不变，不删除历史数据。
- 开发验证：AST直接执行正式键生成/历史选择函数，旧项目精确读取保留，新项目面对唯一旧历史严格返回空，畸形投影失败关闭；对话隔离、生产控制面和文档状态`106 passed`，无失败无跳过。Python编译通过。
- 独立软件测试：通过。冻结提交`a11efb9`复跑精确项目/会话键、唯一旧历史拒绝、畸形投影失败关闭、生产控制面和文档状态`106 passed`，无失败无跳过；Python编译通过。未修改或删除正式历史数据。
- 最终只读稽查：通过。读取只使用`tenant_id + user_id + current_project + session_id`精确键；不存在“唯一记录”或跨project回退，缺失/畸形统一空列表。旧项目按自身键仍可读取，保存接口未变且未执行删除；前端项目缓存无法再被服务端其他项目历史反向污染。
- 关闭时间：2026-08-12（Asia/Shanghai）。下一状态：已关闭；当前代码范围仅余BUG057真实H3受CUDA执行提供方强依赖阻塞。
...
- 最终只读复稽查：通过。baseline与variant只在权威completed且媒体URL非空时回填，失败保持失败；每个await后先验证project+session，持久入口显式携带相同身份，同项目旧session晚到不能修改内存或项目存储。轮询只在资产页和需要对账时运行，不主动重新生成或伪造成功。
- 关闭时间：2026-08-12（Asia/Shanghai）。下一状态：已关闭；进入BUG041历史对话资源隔离分析。
...
- 第十一项稽查首败与整改：项目版本读取/列表虽然接收tenant/user/project，但原实现只按project_id匹配版本文件，完全忽略tenant/user；知道项目ID和版本ID即可跨所有者读取包含完整项目快照的历史版本。现读取任何版本文件前先用三字段从当前项目库验证所有权，不存在或异主统一返回project_not_found。
- 第十一项自动测试与稽查：新增契约证明所有者门禁严格位于单版本读取及目录枚举之前，结合项目存储恢复、生产控制和运行任务隔离关联`93 passed`。只读确认前端已有三字段调用契约，创建与回滚原有所有者门禁不变。
- 第十二项稽查首败与整改：3D任务状态读取只凭job UUID返回完整请求、阶段、错误和候选结果，未复核tenant/user/project；生成记录已持久完整身份但查询未使用。现状态端点强制三字段完整身份并复用统一`_job_matches_scope`，异主UUID按不存在处理；轮询服务与唯一调用点同步传当前项目身份及既有session晚到围栏。
- 第十二项自动测试与稽查：状态端点所有者门禁、服务签名和查询参数新增契约覆盖，结合3D全生命周期及生产控制关联`108 passed`。只读确认生成请求既有`productionTaskContext`提供完整身份，前端只改两行必要调用契约并保留用户其余未提交界面现场。
- 第十三项稽查首败与整改：系统AI异步状态原只凭job UUID读取，未绑定架构要求的project/session所有者边界。现状态查询强制tenant/user/project/session四字段，并与任务创建时持久context精确比较；异主或旧会话UUID不可读，轮询同步当前身份。
- 第十三项自动测试与稽查：新增四字段完整性、持久context精确匹配及服务签名契约，结合生产控制关联`88 passed`。只读确认状态过滤发生在返回任务内容之前，旧会话轮询只会失败关闭且不会写回新会话。
- 第十四项稽查首败与整改：图片结果查询按全局request_name或job UUID匹配，未绑定tenant/user/project；同名资产可跨项目串线，异主查询还可能触发孤儿任务失败写入。现强制完整身份并在选取任务前复用统一scope过滤，五个恢复/轮询调用同步当前项目身份。
- 第十四项自动测试与稽查：新增身份门禁、过滤早于恢复副作用及服务签名契约，结合图片运行、持久投影和生产控制关联`108 passed`。只读确认所有晚到写回仍受调用点既有project/session围栏，异主任务不会进入孤儿检测。
- 第十五项稽查首败与整改：平台通用`TaskService`宣称身份作用域，但队列读取只校验tenant，导致同租户identity-2可列出、读取、取消或恢复identity-1任务。现队列list/get增加可选identity_id精确过滤，服务层所有公开读取与生命周期操作统一传当前身份；保留底层受信调用兼容。
...
- 第十八项稽查首败与整改：系统AI状态GET每次读取都把heartbeat_at覆写为当前时间，而30分钟子进程执行期间没有真实心跳持久化；卡死任务会被无限伪装“心跳正常”，刷新也无法判断真实活性。现执行线程启动独立5秒持久心跳并在终态finally停止，状态接口只导出存储事实。
- 第十八项自动测试与稽查：新增真实heartbeat循环、终态回收及状态接口禁止伪造当前时间的契约，结合系统AI身份/幂等及生产控制关联`90 passed`。只读确认心跳线程只更新running任务，异常与成功均join，不改变30分钟硬超时。
- 第十九项稽查首败与整改：平台实时TaskProgress投影仍以tenant/project/task为键，列表和详情不校验identity；即使任务队列已隔离，同租户异身份仍可从事件投影读取进度。现投影模型、存储键、get/list统一加入identity_id，事件创建者身份成为不可绕过作用域。
- 第十九项自动测试与稽查：新增同租户异身份列表为空、详情拒绝动态测试，结合任务队列/服务及短剧共享任务集成共`16 passed`。只读确认跨租户边界不变，同一task ID在不同身份下不会互相覆盖。
- 第二十项稽查首败与整改：ProviderAuditLedger只记录tenant/project且list也只按两字段过滤，无法满足provider调用审计的用户所有者隔离；同租户同项目不同用户会共享费用、产物和失败证据。现审计记录强制user_id，创建缺任一owner字段失败关闭，列表按tenant/user/project精确过滤。
- 第二十项自动测试与稽查：同用户读取命中、同租户异用户为空及缺所有者门禁结合生产provider集成共`5 passed`。只读确认请求只保存哈希、不扩大敏感输入持久化，成本与产物校验字段保持不变。
- 第二十一项稽查首败与整改：服务重启把所有running系统AI任务直接改queued并重新执行，但Codex子进程不是可检查点恢复任务；崩溃可能发生在文件修改后、状态提交前，自动重跑会重复副作用。现只重启从未开始的queued任务，历史running明确失败并提示显式重试，completed保持不变。
- 第二十一项自动测试与稽查：动态恢复矩阵证明queued唯一重启、running失败关闭、completed不变且单次持久保存；结合真实心跳、身份/幂等及生产控制关联`91 passed`。只读确认不删除现场与输出，避免不可判定副作用的自动重放。
- 第二十二项稽查首败与整改：助手历史、草稿和会话共享同一JSON，但四条HTTP路径均无锁执行load→修改→atomic replace；并发保存不同键时最后写者会覆盖先写者的完整store，形成跨会话数据丢失。现建立统一RLock覆盖四条读取/读改写事务，磁盘原子替换继续作为文件边界。
- 第二十二项自动测试与稽查：契约证明history read/save、draft及sessions四路共用同一锁，结合跨项目对话隔离、系统AI身份及生产控制关联`91 passed`。只读确认锁不包围模型或网络调用，不扩大长耗时临界区。
- 第二十三项稽查首败与整改：资源列表/媒体读取/创建/删除共享resources.json，但并发创建或删除均无锁load→修改→replace，会丢失其他请求刚提交的资源记录。现统一RLock覆盖两条读取快照及创建/删除完整读改写事务，二进制写入仍在锁外避免阻塞列表。
- 第二十三项自动测试与稽查：四条资源路由共享锁契约、所有者媒体门禁及生产控制关联`91 passed`。扩大前端关联出现2项失败来自用户并行未提交界面重构删除旧函数/新增恢复语义，与本资源锁diff无关；未修改或暂存该现场。只读确认资源文件路径与数据校验保持。
- 第二十四项稽查首败与整改：助手history read/save、draft及sessions虽然用四字段拼key，但缺字段时仍生成含空段的共享键，多个不完整请求可落到同一全局会话并互相读取/覆盖。现四条路由在任何存储读取前强制tenant/user/current_project/session完整，缺失统一400失败关闭。
- 第二十四项自动测试与稽查：四路身份门禁数量、字段集合及错误契约结合共享锁、跨项目历史隔离、系统AI状态与生产控制关联`92 passed`。只读确认合法前端`assistantContext`已始终提供完整字段，无兼容降级。
- 第二十五项稽查首败与整改：主助手、同步系统AI和自动路由三个执行入口仍接受缺失项目/会话身份，会把空上下文发送给模型或Codex，绕过同组历史与异步任务已建立的隔离。现三入口在任何模型、搜索、路由或子进程副作用前复用四字段conversation门禁。
...
- 第三十九项稽查首败与整改：AgentMapper双向一对一绑定在两张无锁字典上执行检查后双写；并发绑定可各自通过检查并留下skill→agent与agent→skill不一致。现RLock覆盖完整检查与双向提交以及双向读取。
- 第三十九项自动测试与稽查：Agent映射、注册及平台启动关联`10 passed`；Python编译与diff门禁通过，双向读取只观察完整映射。
- 第四十项稽查首败与整改：ModelRegistry注册、启停、列表和能力选择无锁；并发注册同model_id可绕过冲突检查，select遍历期间启停/注册还可能抛出字典变化或选到撕裂视图。现统一RLock覆盖注册、启停与读取，并在锁内复制/构建候选快照后选择。
...
- 第六十五项稽查首败与整改：ProviderService注册配置同样只检查settings顶层键，嵌套transport.headers等位置可持久明文authorization/credential。现注册时递归扫描Mapping及序列，敏感配置在进入配置快照前失败关闭。
- 第六十六项稽查首败与整改：底层ProviderAdapterDefinition仍可在settings嵌套结构携带authorization/credential，绕过管理服务门禁后进入运行注册表快照。现适配器数据模型自身执行同等递归敏感字段校验，使所有构造入口在注册前失败关闭。
- 第六十七项稽查首败与整改：ModelDefinition.settings由管理API直接接受并进入模型注册表/响应，未执行敏感字段过滤，嵌套authorization可成为长期内存配置。现模型定义工厂递归拒绝敏感键，保持密钥仅使用provider外部secret reference的架构边界。
- 第六十八项稽查首败与整改：内存WorkloadRouter会校验worker身份和容量，但共享SQLite WorkerRegistry heartbeat可直接持久空scope、空能力、零容量或非法generation；其他实例会把畸形发现事实载入路由。现持久发现入口在写事务前执行同等身份、资源和容量门禁，非法快照零落盘。
...
- 第一百五十一项稽查首败与整改：ProductionExtensionRegistry信任集合和register布尔/metadata依赖静态类型，required_methods还会把数字字符串化为方法名。现构造及注册在factory探针和共享注册表前完整失败关闭。
...
- 第一百九十七项稽查首败与整改：IdentityContext的_required与identity_kind直接对运行时值strip，数字身份会泄漏AttributeError，require_tenant亦无法按统一契约失败关闭。现全部scope字段共享字符串门禁。
...
- 第二百二十四项稽查首败与整改：同代且同时标的重复worker心跳仍可以不同负载值覆盖快照，跨实例最终状态取决于抵达顺序。现相同时标只返回已接受的权威快照。
...
- 第二百五十三项稽查首败与整改：StoryBible事实与实体JSON仍接受NaN，任意对象又会在SQLite事务执行途中才失败，无法保证权威故事事实的标准可重放边界。现完整输入在事务和任何写入前强制标准JSON。
- 第二百五十三项自动测试与稽查：NaN与任意对象故事输入动态拒绝且事实/实体表保持空；故事圣经与生产控制关联回归、Python编译与diff门禁通过。
- 第二百五十四项稽查首败与整改：短剧TextArtifact用默认JSON往返规范化，允许NaN进入不可变文本产物和校验指纹，任意对象则泄漏底层TypeError。现所有需求、提纲、剧本、分镜与资产目录产物统一强制标准JSON领域错误。
...
- 第二百五十九项稽查首败与整改：共享`atomic_write_json`在验证payload前已创建目标父目录及临时文件，坏JSON虽不替换正式文件仍产生文件系统副作用。现标准JSON完整序列化前置到路径解析和目录创建之前。
...
- 第二百六十三项稽查首败与整改：QueuedTask同样只冻结payload顶层，enqueue后调用方仍可篡改嵌套任务输入，破坏幂等重放和队列执行事实。现以已验证标准JSON生成独立深快照。
...
- BUG072 / M9.198 / 架构横向稽查第二百九十一项（已关闭，2026-08-12）：LangGraph兼容流水线加载artifact manifest时把任意键值强制转成字符串，NaN、未知stage或数值路径可进入恢复状态。现严格拒绝非标准JSON、非canonical stage及非字符串空路径；三类持久反例动态失败关闭。关联`10 passed, 3 subtests passed`，Python编译、文档状态及diff门禁通过。
- BUG072 / M9.198 / 架构横向稽查第二百九十二项（已关闭，2026-08-12）：兼容ShortDramaPipeline恢复只校验user_id，持久checkpoint内tenant/project/run、生命周期、索引和artifact映射可被篡改后进入错误Graph作用域。现严格绑定请求与持久四元身份，并验证生命周期、索引范围及canonical artifact键值；五类篡改动态失败关闭。关联`11 passed, 8 subtests passed`，Python编译、文档状态及diff门禁通过。
- BUG072 / M9.198 / 架构横向稽查第二百九十三项（已关闭，2026-08-12）：私有部署SQLite状态与队列读取使用宽松JSON解析，且queue claim在验证payload前即可准备running转换。现持久读取严格拒绝NaN/坏JSON并要求queue object；解析失败显式rollback，任务保持pending。故障注入动态证明坏状态拒绝且坏队列无生命周期跃迁。关联`11 passed, 21 subtests passed`，Python编译、文档状态及diff门禁通过。
- BUG072 / M9.198 / 架构横向稽查第二百九十四项（已关闭，2026-08-12）：DurableTaskRepository权威任务与projection outbox读取仍宽松接受NaN或非对象JSON，坏持久记录可进入运行任务API或投影重放。现两路径共享严格标准JSON对象解码门禁；NaN权威行与数组outbox故障注入均失败关闭。关联`15 passed, 18 subtests passed`，Python编译、文档状态及diff门禁通过。
...
- BUG072 / M9.198 / 架构横向稽查第三百零一项（已关闭，2026-08-12）：DeliveryPipeline对compose/repair输出及review决策只做属性/真值检查，任意对象、文本content或非字符串issues会泄漏异常或进入交付事实。现强制DeliveryOutput/ReviewDecision精确类型、bytes媒体、bool标志和非空字符串issues；四类伪返回动态失败关闭。关联`13 passed, 7 subtests passed`，Python编译、文档状态及diff门禁通过。
- BUG072 / M9.198 / 架构横向稽查第三百零二项（已关闭，2026-08-12）：ShortDramaProviderBindings直接索引provider媒体/交付字段并把approved用`is True`静默改写，缺字段、伪布尔或坏时间线会泄漏异常或被误解释；同时媒体bytes无法穿越ProviderAdapter标准JSON边界。现验证registry/routes、捕获字段错误、强制媒体/审核类型，并在统一边界显式base64封装bytes；坏媒体、伪审核、伪registry及真实bytes调用动态验证通过。关联`19 passed, 14 subtests passed`，Python编译、文档状态及diff门禁通过。
- BUG072 / M9.198 / 架构横向稽查第三百零三项（已关闭，2026-08-12）：ShortDramaProviderBindings媒体JSON转换把任意Mapping键强制`str`，整数键可与字符串键碰撞并静默改写请求事实。现递归要求所有provider input键为非空字符串，禁止隐式键转换；顶层与嵌套整数键动态失败关闭。关联`19 passed, 16 subtests passed`，Python编译、文档状态及diff门禁通过。
...
- BUG072 / M9.198 / 架构横向稽查第三百一十八项（已关闭，2026-08-12）：WorkerRegistry新heartbeat已规范化，但历史SQLite payload读取仍原样发布带边界空白的身份，list过滤与reserve路由可继续漏读或产生幽灵worker。现严格持久解析后统一重建canonical WorkerSnapshot；历史空白记录动态恢复并精确匹配作用域。关联回归、Python编译与diff门禁通过。
...
- BUG072 / M9.198 / 架构横向稽查第三百二十三项（已关闭，2026-08-12）：ProviderAuditLedger请求指纹虽拒绝非标准值，却仍允许JSON把整数/空白键静默改写，可能为不同真实请求生成相同审计哈希；敏感检查也依赖强制字符串化。现哈希前递归要求非空字符串键并同步敏感过滤；三类伪键动态失败关闭。关联回归、Python编译与diff门禁通过。
