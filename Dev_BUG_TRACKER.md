...
- 当前整改：新增显式`video_provider_id`（`wan2.2-ti2v-5b`/`minimax-h3-ref2va`），用户选择Wan时即使命令携带H3参考也不再误入CUDA阻断；Wan恢复30步与704×1280竖屏等面积参数，completed持久精确provider。未知provider及H3缺少双参考失败关闭，待重载后真实第三镜复测。
