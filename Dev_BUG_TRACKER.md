...
- 第五十七项稽查首败与整改：内存路由和SQLite worker发现仅拒绝更旧generation，同generation乱序晚到heartbeat可覆盖更新容量、负载和时间戳，违背epoch内晚到响应隔离。现同generation强制heartbeat_at单调，内存路由显式拒绝倒退，SQLite CAS忽略倒退快照。
