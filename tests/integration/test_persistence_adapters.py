...
class PersistenceAdaptersTest(unittest.TestCase):
 ...
 def test_object_traversal_is_rejected(self):
  with tempfile.TemporaryDirectory() as d:
   store=LocalObjectStore(Path(d))
   for tenant,key in (("t","../../x"),("a/..","x"),("t","a/../x"),("t","/x")):
    with self.subTest(tenant=tenant,key=key),self.assertRaises(PersistenceError):store.put(tenant,key,b"x")
