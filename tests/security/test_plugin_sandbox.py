...
class PluginSandboxTest(unittest.TestCase):
 def test_policy_and_process_runtime_contracts_are_rejected(self):
  ...
  for command,timeout in (("/usr/bin/true",30),(iter(("/usr/bin/true",)),30),(("/usr/bin/true",),True)):
 ...
 def test_escape_unlisted_host_and_process_are_denied(self):
  for call in (lambda:self.broker.resolve_plugin_file("../../secret"),lambda:self.broker.validate_network("https://evil.example"),lambda:self.broker.validate_network("https://user:pass@api.example.com"),lambda:self.broker.validate_network("https://api.example.com:8443"),lambda:self.broker.run_process(("/bin/sh","-c","id"))):
